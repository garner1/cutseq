...
#     if [ "$mode" == "PE" ]
#     then
#     	bwa mem -v 1 -t $numbproc $refgen "$in"/barcode_"$barcode".fq "$in"/barcode_"$barcode"-r2.fq | samtools sort -@ 8 -T ${aux}/${experiment} > "$aux"/"$barcode".all.bam
#     fi
    
#     if [ "$mode" == "SE" ]
#     then
#     	bwa mem -v 1 -t $numbproc $refgen "$in"/barcode_"$barcode".fq | samtools sort -@ 8 -T ${aux}/${experiment} > "$aux"/"$barcode".all.bam
#     fi
...
#     	/usr/local/share/anaconda3/bin/alfred qc -r /home/garner1/Work/genomes/Homo_sapiens.GRCh37.dna.primary_assembly.fa/GRCh37.fa \
# 					      -b /home/garner1/Work/dataset/agilent/S07604715_Covered.woChr.bed \
# 					      -j "$out"/"$barcode".deduplicated.q"$quality".json.gz -o "$out"/"$barcode".deduplicated.q"$quality".tsv.gz \
...
#     fi
# done
git add -A && git commit -m "git repo updated after processing " -m "`$commitDate`" && git push origin development
